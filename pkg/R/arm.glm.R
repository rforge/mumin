...
function(object, y, x, wt) {
	fam <- object$family
	nobs <- NROW(x)
	n <- if(NCOL(y) == 1) rep.int(1, nobs) else rowSums(y)
...
function(object, R = 250, weight.by = c("aic", "loglik"), trace = FALSE) {
	...
	combinTerms <- lapply(0L:(2^nterms - 1L), function(j) as.logical(intToBits(j)[1L:nterms]))
	combinTerms <- combinTerms[vapply(combinTerms, formula_margin_check, FALSE, deps)]
	## combinPredictors
	combin <- vapply(combinTerms, function(x, assign) assign %in% c(0, which(x)), logical(ncol(mm)),
	...
	for(iter in 1L:R) {
		...
		for(u in 1L:maxtrials) {
			...
			for(k in seq.int(nModels)) {
				...
				ic[k] <- glmfit_aicloglik(fit1, vy2, x2[, combin[, k], drop = FALSE], prwts2)[1L]
			}
			traceinfo("\n")
			if(!problem) break
		}
	...
	msTable <- 	matrix(NA_real_, nrow = nModels, ncol = 6L,
		dimnames = list(1L:nModels, c("df", "logLik", "AIC", "delta", "weight",
									  "ARM weight")))
	## coefArray(model, c(coef, se, df), coefficients)
	coefArray <- array(NA_real_, dim = c(nModels, 3L, length(cfnames)),
		dimnames = list(1L:nModels, c("Estimate", "Std. Error", "df"), cfnames))
	for(k in seq.int(nModels)) {
		fit1 <- glm.fit(x[, combin[, k], drop = FALSE], y, family = fam)
		coefArray[k, , combin[, k]] <-
			rbind(t(summary.glm(fit1)$coefficients[, 1L:2L, drop = FALSE]), fit1$df.residual)
		msTable[k, c(3L, 2L, 1L)] <-
			glmfit_aicloglik(fit1, fit1$y, x[, combin[, k], drop = FALSE], fit1$prior.weights)
	}
